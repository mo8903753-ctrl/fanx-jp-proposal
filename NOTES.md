...
## 目标双档（v2 用户拍板 2026-08-21：合计压到 17.x% / 18%）
...
## 预算 $500K 档（v2：17.8%，上线广告为唯一调节行）
预热广告 5.25%=$26,250 · 上线广告 11.50%=$57,500 · 媒体&KOL 0.62%=$3,100 · PR TIMES ¥33,000=$206 · 线下展示 ¥250,000=$1,563 · 其他工具 ¥61,803=$386 → 合计 $89,005（17.8%）
$1M 档（18.0%）：53,000 / 116,600（11.66%）/ 7,000 / 300 / 2,500 / 600 → $180,000
预热 Leads：$26,250 ÷ CPL $3–4 ≈ 6,600–8,800 条；预热转化取 5%（区间 3–8%）
预算章末有免责小字：运营测算、不构成销售结果承诺，按 CPL/Leads/首日 GMV/ROAS 持续调整。
...
## 章节结构（v2 现行 11 章；合作模式章与双档试算章 2026-08-21 用户拍板删除：客户没问模式、全部代运营口径、净回款不展示）
01 封面 / 02 品类判断·产品证明 / 03 竞品与比较（日本已上架：UGREEN NASync 等） / 04 目标拆解 / 05 定价档位 / 06 受众 / 07 营销预算+KOL（id="n-kol"）/ 08 时间线（60/90 天两档）/ 09 公司 / 10 案例资源 / 11 可行性+收尾
注意：锚点 id 未随重编号改动（c05=04 目标拆解、c07=05 定价、c08~c13=06~11），改交叉引用时以 id 为准。
...
- 预算合计 16.31%/18.34% → 17.8%/18.0%（只动上线广告一行：$50,000→$57,500、$120,000→$116,600）
- 预算章 KPI 后加测算免责小字
- 定价阶梯 .pr 溢出修复：去掉 nowrap，JPY 小字改 display:block 独立行（00-head.html；ego-browser 复验 overflow=false×6）
- humanizer-zh 清扫：格局→情况、正文 6 处破折号揭示→逗号/冒号（保留 bigquote 与引语出处 2 处）
- 教训：ego-browser openOrReuseTab 复用旧 tab 会拿缓存页面，验证修改必须 gotoAndWait 带 ?v= 强制刷新
